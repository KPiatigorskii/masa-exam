...
11. **Authentication & Authorization** *=> Authentication is a proccess of verifying user and sending token in our case. Authorization is a proccess of checking right/permissions of user for some actions 

12. **Stored procedure** *=> It's like a big function which can use several queries in one file. This "function" stored in db and can run from different sources

13. **Git rebase** *=> When we call git rebase we rewrite commit story of current branch and adding to the base branch new commits

14. **Generics** *=> It's a pattern, which you can use when you must use class or methods in several situations with different return types of value

15. **Middleware** *=> It's some functions which we can use between user send request and our server run usual handling.
...
16. **You received a bug stating the following: "Intermittently the following method results in a system getting stuck." You're required to find and fix the problem in this method:** *=> 10 points*
		...
		public static addMonths(date: Date, value: number): Date {
		    ...
		    if (expectedMonth > 11) { // function getMonth() return values from 0 to 11, so in this line we must check "> 11"
...
19. **Explain the following piece of code:** *=> 5 points*
		...
		this function return STRING of "shifted" value from passed value with passed value of zeroes
		for example padleft(23,6) give us string "000023"
